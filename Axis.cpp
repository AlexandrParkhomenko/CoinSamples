// good link https://coin3d-ru.livejournal.com
...
//#		root->addChild(drawLines(ver, 2,colorbig, itoa(i*scale,text,10), 2.2, SbVec3f(x == 1 ? i*scale : z == 1 ? 0 : -2.7, y == 1 ? i*scale : -2.2, z == 1 ? i*scale : 0)));
	...
	for (int i = 0; i <X_MAX; i++)
	{
		...
		root->addChild(drawLines(ver, 2,colorsmall, (char*)" ", 1.8, SbVec3f(0,0,0)));
	}
}
int main(int, char **argv)
{
	...
	static QWidget* myQtdow = SoQt::init("Axis"); // pass the app name
	...
	root->ref();
	root->addChild(drawLines(xvert,2, SbColor(1,0,0), (char*)"X-axis", 4.0, SbVec3f(X_MAX/2,-5,0)));
	root->addChild(drawLines(yvert, 2,SbColor(0, 1,0), (char*)"Y-axis", 4.0,SbVec3f(-8, Y_MAX/2, 0)));
	root->addChild(drawLines(zvert,2, SbColor(0, 0, 1), (char*)"Z-axis", 4.0,SbVec3f(0, -5, Z_MAX/2)));
	root->addChild(drawLines(xyvert, 12,SbColor(0, 1, 1), (char*)"", 2.0, SbVec3f(0, 0,0)));
	//steps
